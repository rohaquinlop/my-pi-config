...
export default function skillScout(pi: ExtensionAPI) {
	pi.on("before_agent_start", async (event, _ctx) => {
		return { systemPrompt: event.systemPrompt + "\n\n# Skill Scout\nWhen you notice a repeated project workflow that would benefit from a reusable skill, call skill_scout_record. Do not create or approve skills unless the user explicitly asks; Skill Scout is confirm-first. When drafting, reviewing, adapting, or approving skills, use the skill-development skill guidance if available instead of relying only on the generic draft template." };
	});
	...
	pi.registerCommand("skill-scout", {
		...
		handler: async (args, ctx) => {
			...
			if (cmd === "draft") {
				...
				ctx.ui.notify(`Drafted: ${file}. Review with /skill:skill-development, then approve with /skill-scout approve ${p.key}`, "info");
				return;
			}
